...
import { useTradeIntentsEdge } from "@/hooks/useTradeIntentsEdge";
import { useBotConfigEdge } from "@/hooks/useBotConfigEdge";
...
import { Check, X, Clock, AlertTriangle, PlayCircle, Ban, RefreshCw, Plus, Settings, Home, FileText } from "lucide-react";
...
async function restHeaders(prefer?: string) {
...
export default function IntentsEdge() {
  const { user } = useAuth();
  const navigate = useNavigate();
  const { intents, loading, creating, createIntent, refresh } = useTradeIntentsEdge(user?.id || null);
  const { config, loading: configLoading, updateConfig, saving } = useBotConfigEdge(user?.id || null);
  ...
  const castVote = useCallback(async (intentId: string, vote: "approve" | "deny", confidence = 1, rationale?: string) => {
    if (!user?.id) return false;
    setVoting(intentId);
    try {
      const response = await fetch(`${restBaseUrl()}/trade_intent_votes`, {
        method: "POST",
        headers: await restHeaders("return=minimal"),
        body: JSON.stringify({
          intent_id: intentId,
          user_id: user.id,
          vote,
  ...
  const cancelIntent = useCallback(async (intentId: string) => {
    if (!user?.id) return false;
    try {
      const response = await fetch(`${restBaseUrl()}/trade_intents?id=eq.${intentId}&user_id=eq.${user.id}`, {
        method: "PATCH",
        headers: await restHeaders("return=minimal"),
          ...
          <Dialog open={newIntentOpen} onOpenChange={setNewIntentOpen}>
            ...
            <DialogContent>
              ...
              <div className="space-y-4">
                ...
                <Button className="w-full" onClick={handleCreateIntent} disabled={creating}>
          ...
          {configLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="flex items-center justify-between p-3 border rounded-lg">
                ...
                <Switch
                  checked={config.voting_enabled ?? true}
                  onCheckedChange={v => updateConfig({ voting_enabled: v } as Partial<typeof config>)}
                  disabled={saving}
                />
              </div>
              ...
              <div className="flex items-center justify-between p-3 border rounded-lg">
                ...
                <Switch
                  checked={config.auto_approve_enabled ?? false}
                  onCheckedChange={v => updateConfig({ auto_approve_enabled: v } as Partial<typeof config>)}
                  disabled={saving}
                />
              </div>
              ...
              <div className="flex items-center justify-between p-3 border rounded-lg bg-amber-50 dark:bg-amber-950">
                ...
                <Switch
                  ...
                  onCheckedChange={v => updateConfig({ dry_run: v } as Partial<typeof config>)}
                  disabled={saving}
                />
              </div>
              ...
              <div className="p-3 border rounded-lg">
                ...
                <Input
                  type="number"
                  value={config.max_notional_per_order_usd ?? 100}
                  onChange={e => updateConfig({ max_notional_per_order_usd: parseFloat(e.target.value) || 0 } as Partial<typeof config>)}
                  min={1}
                  step={10}
                  className="mt-1"
                />
              ...
              <div className="p-3 border rounded-lg">
                ...
                <Input
                  type="number"
                  value={config.max_open_orders ?? 5}
                  onChange={e => updateConfig({ max_open_orders: parseInt(e.target.value, 10) || 0 } as Partial<typeof config>)}
                  min={1}
                  max={20}
                  className="mt-1"
                />
                    ...
                    <div className="space-y-1">
                      ...
                      {intent.status === 'failed' && intent.metadata?.execution_result && (
                        <p className="text-xs text-red-500">
                          Error: {(intent.metadata.execution_result as { error?: string })?.error || 'Unknown'}
                        </p>
                      )}
                    </div>
                  </div>
