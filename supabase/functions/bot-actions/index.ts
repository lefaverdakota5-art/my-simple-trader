...
serve(async (req) => {
  ...
  try {
    ...
    // Process deposit from Chime to trading account
    if (action === "deposit") {
      ...
      const currentBalance = stateData?.balance ?? 0;
      const newBalance = currentBalance + amount;

      // Update or insert trader_state with new balance
      ...
      console.log(`Deposit successful: $${amount} for user ${userId}, new balance: $${newBalance}`);
      return jsonResponse({ success: true, newBalance });
    }

    // Process withdrawal from trading account to Chime
    if (action === "withdraw") {
      const amount = Number(body?.amount ?? 0);
      if (amount <= 0) {
      ...
      console.log(`Withdrawal successful: $${amount} for user ${userId}, new balance: $${newBalance}`);
      return jsonResponse({ success: true, newBalance });
    }
