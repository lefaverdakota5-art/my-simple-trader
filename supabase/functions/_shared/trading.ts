...
export function normalizeKrakenPair(symbol: string): string {
  ...
  const mapped = raw
    ...
    .replace("BTC-USDT", "XBT/USDT")
    .replace("BTC-USD", "XBT/USD");
...
export async function fetchTicker(symbol: string): Promise<{ last: number; ask: number; bid: number }> {
  ...
  const first = Object.values(result)[0] as Record<string, string[]> | undefined;
  if (!first) {
    throw new Error(`No ticker returned for ${symbol}`);
  }
  return {
    last: Number(first.c?.[0] || 0),
    ask: Number(first.a?.[0] || 0),
    bid: Number(first.b?.[0] || 0),
  };
}
...
export async function krakenPrivateRequest(
  ...
  const secretBytes = decodeBase64(apiSecret);
  ...
  if (!response.ok || data.error?.length) {
    throw new Error(data.error?.join(", ") || `Kraken request failed with status ${response.status}`);
  }
  return data.result;
}
...
export async function persistExecution(
  ...
  const { error: orderError } = await supabaseAdmin.from("trading_orders").insert(tradingOrderPayload);
...
export async function executeIntent(supabaseAdmin: any, intent: TradeIntentRow, config: BotConfigRow, keys: UserExchangeKeysRow): Promise<ExecutionResult> {
  ...
  if (openOrders >= config.max_open_orders) {
    return {
      success: false,
      mode: "dry_run",
      error: `Open order cap reached (${openOrders}/${config.max_open_orders})`,
      requested_symbol: intent.symbol,
      requested_side: intent.side,
      requested_notional_usd: intent.notional_usd,
      requested_quantity: intent.quantity,
  ...
  if ((intent.notional_usd || 0) > config.max_notional_per_order_usd) {
    return {
      success: false,
      mode: "dry_run",
      error: `Notional exceeds limit (${intent.notional_usd} > ${config.max_notional_per_order_usd})`,
      requested_symbol: intent.symbol,
      requested_side: intent.side,
      requested_notional_usd: intent.notional_usd,
      requested_quantity: intent.quantity,
  ...
  const txid = Array.isArray((raw as Record<string, unknown>).txid)
    ? String((raw as Record<string, unknown>).txid?.[0])
    : undefined;
